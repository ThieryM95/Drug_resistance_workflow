...
#                                                 ##
# author: andrewjames.shattock@swisstph.ch        ##
# adapted: thiery.masserey@swisstph.ch            ##
####################################################
...
dataFrame <- function(x, colNames = NULL, rowNames = NULL, ...) {
  ...
  } # Rename column headers if need be
  return(df)
}
...
#----------------------------------------------------
# In the perinea setting 
adjust_EIR <- function(eir, access) {
  
  # load the emulator
  trained_model <- readRDS(paste0(file.path("/scicore/home/penny/masthi00", "wf_esthablishment", "EIR_gp_no_seasonality_2.RData")))
  
  # predict the input EIR to simulat the wanted EIR in a setting that has a specific treatment access
  predict_eir <- predict(x = as.matrix(data.frame(eir, access)), object = trained_model)
  predict_eir <- predict_eir$mean
  
  # return the adjusted eir
  return(predict_eir)
}

# In the seasonal setting
adjust_EIR_seasonality <- function(eir, access) {
  
  # load the emulator
  trained_model <- readRDS(paste0(file.path("/scicore/home/penny/masthi00", "wf_esthablishment", "EIR_gp_seasonality_2.RData")))
...
adjust_Fitness <- function(fitness) {
  ...
  # load the emulator
  trained_model <- readRDS(paste0(file.path("/scicore/home/penny/masthi00", "wf_esthablishment", "Fitness_gp.RData")))
  
  # predict the maximum killing rate needed to model a specific fitness cost
  NEW_DATA <- NULL
  NEW_DATA$Scenario_liste.Indicator <- fitness
  predict_eir <- predict(trained_model, newdata = NEW_DATA)
  
  # return the maximum killing rate
  return(predict_eir)
}
...
# # Load, data
# ---------------------------------------------------------
format_input_data <- function(pm) {
  message(" > Loading input data")

  # Load large input file from BMGF (program coverage 2000-2040)
  input_df <- read.csv(pm$pth$input_file, header = TRUE, sep = ";")
}

# ---------------------------------------------------------
# Generate full list of scenarios to simulate.
# ---------------------------------------------------------
generate_scenario_list <- function(pm) {
  message("  - Creating scenario list")

  scenario_list <- input_df$Scenario_Name

  n_jobs_total <- length(scenario_list)
